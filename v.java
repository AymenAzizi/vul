...
// Now you can use the sanitized output
```

This code uses the ProcessBuilder class to create a new process and execute the command, which is safer than using Runtime.getRuntime().exec(). Additionally, it sanitizes the user input by only allowing alphanumeric characters and spaces, which should maintain the original functionality.
